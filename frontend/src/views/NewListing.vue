<template>
  <div class="column">
    ...
    <input
      type="number"
      v-model="distanceToCampus"
      placeholder="Distance to Campus (miles)"
    />
...
export default {
  ...
  methods: {
    getAllUsers() {
      axios
        .get("http://" + window.location.hostname + ":3030/api/users/")
        .then((response) => {
    ...
    addListing() {
      console.log("sent");
      axios
        .post("http://" + window.location.hostname + ":3030/api/listings/", {
          name: this.name,
